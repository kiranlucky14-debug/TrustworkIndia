...
export default function LoginPage() {
  ...
  const verifyOTP = async () => {
    ...
    try {
      ...
      if (isNew) { payload.name = name || activeRole.demo.name; payload.role = role }
      const { data } = await api.post('/auth/verify-otp', payload)
      login(data.token, data.user)
      toast.success(`Welcome${isNew ? ', ' + data.user.name : ' back'}! 🎉`)
      navigate('/dashboard')
    } catch (err) {
      const msg = errMsg(err)
      if (msg.includes('Name required')) { setIsNew(true); toast('Please fill your name to register', { icon: 'ℹ️' }) }
      ...
      <div className="left-panel" style={{ flex: '0 0 50%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '44px 52px', position: 'relative' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {[['🔒','Escrow Protected','#14b8a6'],['⚡','Instant UPI Payout','#fbbf24'],['⚖️','Dispute Resolution','#818cf8']].map(([icon, label, color]) => (
