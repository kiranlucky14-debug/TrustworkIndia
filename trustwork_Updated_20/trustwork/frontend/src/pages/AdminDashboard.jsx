...
function PayoutQueuePanel({ payouts, payoutFilter, setPayoutFilter, setPayouts, onRefresh }) {
  ...
  const pendingCount = payouts.filter(p => p.status === 'PENDING_REVIEW').length
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:20 }}>
        {FILTERS.map(f => {
          const cnt = payouts.filter(p => p.status === f.id).length
          const amt = payouts.filter(p => p.status === f.id).reduce((s,p) => s + Number(p.netAmount||0), 0)
          return (
      ...
      {releasing && (
        <ReleaseModal
          ...
          onDone={() => { setReleasing(null); reload(payoutFilter); onRefresh() }}
        />
      )}
    </div>
  )
