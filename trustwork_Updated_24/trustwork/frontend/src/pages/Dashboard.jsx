...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetches = [api.get(url)]
    if (user?.role === 'FREELANCER') fetches.push(api.get('/match/dashboard').catch(() => ({ data: { topJobs: [] } })))
    Promise.all(fetches)
      .then(([jobsRes, matchRes]) => {
        setJobs(jobsRes.data.jobs || [])
        if (matchRes) setMatchedJobs(matchRes.data.topJobs || [])
      })
      .catch(() => {})
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* Matched jobs for freelancer */}
      {isFreelancer && matchedJobs.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display font-semibold text-lg text-white">Matched for You</h2>
            <a href="/jobs" className="text-sm text-brand-400 hover:text-brand-300 transition-colors">View all</a>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {matchedJobs.slice(0,4).map(job => (
              <div key={job.id} className="card p-4 hover:border-brand-500/30 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <div className="font-semibold text-white text-sm truncate flex-1 pr-2">{job.title}</div>
                  <div style={{ padding:'2px 8px', borderRadius:100, background:'rgba(20,184,166,.15)', border:'1px solid rgba(20,184,166,.3)', fontSize:11, fontWeight:700, color:'#14b8a6', flexShrink:0 }}>
                    {job.matchScore}% match
                  </div>
                </div>
                <div className="text-xs text-ink-500 mb-3">Budget: <strong className="text-emerald-400">Rs.{job.budget?.toLocaleString('en-IN')}</strong></div>
                <div className="flex gap-2 flex-wrap mb-3">
                  {(job.skills||[]).slice(0,3).map(js => (
                    <span key={js.skill?.id} style={{ fontSize:10, padding:'2px 8px', borderRadius:100, background:'rgba(255,255,255,.06)', border:'1px solid rgba(255,255,255,.1)', color:'rgba(255,255,255,.55)' }}>
                      {js.skill?.name}
                    </span>
                  ))}
                </div>
                <a href={'/jobs/'+job.id} style={{ fontSize:12, color:'#14b8a6', textDecoration:'none', fontWeight:600 }}>View Job </a>
              </div>
            ))}
          </div>
        </div>
