...
export default function NotificationBell() {
  ...
  const openDropdown = async () => {
    ...
    if (willOpen) {
      ...
      if (btnRef.current) {
        ...
        // Position dropdown: left-aligned from button, but clamped to viewport
        const dropWidth = 340
        let left = rect.left
        // If it would overflow right edge, push it left
        if (left + dropWidth > window.innerWidth - 8) {
          left = window.innerWidth - dropWidth - 8
        }
        // If in sidebar (left < 280), open to the right of sidebar instead
        if (rect.right < 280) {
          left = rect.right + 8
        }
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {/* Dropdown */}
      {open && (
        <div style={{ position: 'fixed', left: dropPos.left, top: dropPos.top, width: 340, maxHeight: 460, overflowY: 'auto', background: '#0e1c2f', border: '1px solid rgba(255,255,255,.1)', borderRadius: 14, boxShadow: '0 24px 60px rgba(0,0,0,.7)', zIndex: 9999, fontFamily: "'DM Sans',system-ui,sans-serif" }}>
          {/* Header */}
