...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    // Pre-populate whatever we already know from the signup step
    const prefill = {}
    if (user.name  && user.name !== 'New User') prefill.name  = user.name
    if (user.email) prefill.email = user.email
    if (user.phone) prefill.phone = user.phone
    if (user.city)  prefill.city  = user.city
    if (user.state) prefill.state = user.state
    if (Object.keys(prefill).length > 0) reg.setMany(prefill)
      ...
      <div style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, padding: '3px 10px', borderRadius: 100,
            background: user?.role === 'CLIENT' ? 'rgba(96,165,250,0.15)' : 'rgba(20,184,166,0.15)',
            color: user?.role === 'CLIENT' ? '#60a5fa' : '#14b8a6', fontWeight: 600 }}>
            {user?.role}
          </span>
        </div>
      </div>
