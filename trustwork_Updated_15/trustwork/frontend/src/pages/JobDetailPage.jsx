  ...
  const fetchJob = async () => {
    try {
      const [jobRes, msRes] = await Promise.all([
        api.get(`/jobs/${id}`),
        api.get(`/jobs/${id}/milestones`).catch(() => ({ data: { milestones:[], summary:{} } })),
      ])
      setJob({ ...jobRes.data, _milestones: msRes.data.milestones || [], _msSummary: msRes.data.summary || {} })
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          {(isClient || isFreelancer) && job.freelancerId && (
            <div>
              {job._milestones?.length > 0 && (
                <div style={{ marginBottom:10, background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.08)', borderRadius:12, padding:'12px 14px' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                    <span style={{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,.38)', textTransform:'uppercase', letterSpacing:'.07em' }}>
                      Milestones ({job._milestones.length})
                    </span>
                    <span style={{ fontSize:11, color:'#14b8a6' }}>
                      {job._msSummary?.released_count || 0}/{job._milestones.length} done
                    </span>
                  </div>
                  {job._milestones.slice(0,3).map((m, i) => {
                    const statusColor = { LOCKED:'#475569', UNLOCKED:'#14b8a6', FUNDED:'#fbbf24', SUBMITTED:'#818cf8', UNDER_ADMIN_REVIEW:'#fb923c', RELEASED:'#34d399', REFUNDED:'#f43f5e' }[m.status] || '#94a3b8'
                    return (
                      <div key={m.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'5px 0', borderBottom: i < Math.min(job._milestones.length,3)-1 ? '1px solid rgba(255,255,255,.05)' : 'none' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:7 }}>
                          <div style={{ width:7, height:7, borderRadius:'50%', background: m.isLocked ? '#475569' : statusColor, flexShrink:0 }}/>
                          <span style={{ fontSize:12, color: m.isLocked ? 'rgba(255,255,255,.4)' : 'rgba(255,255,255,.7)', maxWidth:140, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{m.title}</span>
                        </div>
                        <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                          <span style={{ fontSize:11, fontWeight:600, color: m.isLocked ? '#475569' : statusColor }}>{m.status?.replace('_',' ')}</span>
                          <span style={{ fontSize:11, color:'rgba(255,255,255,.4)' }}>Rs.{Number(m.amount).toLocaleString('en-IN')}</span>
                        </div>
                      </div>
                    )
                  })}
                  {job._milestones.length > 3 && (
                    <div style={{ fontSize:11, color:'rgba(255,255,255,.35)', marginTop:5, textAlign:'center' }}>
                      +{job._milestones.length - 3} more milestones
                    </div>
                  )}
                </div>
              )}
              ...
              >
                {job._milestones?.length > 0 ? 'Manage Milestones' : 'View Milestones'}
              </Link>
            </div>
          )}
