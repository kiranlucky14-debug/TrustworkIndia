...
import CategorySkillPicker, { SKILL_CATEGORIES } from '../../components/CategorySkillPicker'
...
export default function FreelancerRegistration({ reg, phone, onComplete }) {
  ...
  // Skills stored as array of names (strings)
  const [selSkillNames, setSelSkillNames] = useState([])

  const handleSkillChange = (names) => {
    setSelSkillNames(names)
    reg.set('skills', names)
  }
        ...
        <div className="step-card">
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <label className="reg-label" style={{ margin: 0 }}>Skills</label>
              <span style={{ fontSize: 11, color: '#64748b' }}>{selSkillNames.length}/15 selected</span>
            </div>
            <CategorySkillPicker
              selected={selSkillNames}
      ...
      {step === 5 && (
        <div>
          <div className="step-card">
            ...
            {selSkillNames.length > 0 && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ fontSize: 11, color: '#64748b', marginBottom: 8 }}>Skills ({selSkillNames.length})</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                  {selSkillNames.map(name => (
                    <span key={name} style={{ padding: '2px 9px', borderRadius: 100, background: 'rgba(129,140,248,0.12)', border: '1px solid rgba(129,140,248,0.25)', color: '#818cf8', fontSize: 11 }}>{name}</span>
                  ))}
                </div>
              </div>
            )}
