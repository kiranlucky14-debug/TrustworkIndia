...
export default function AgreementPage() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',system-ui,sans-serif", color: '#f8fafc', maxWidth: 720 }}>
      ...
      {!ag && isFreelancer && (
        <div style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', borderRadius: 14, padding: 28, textAlign: 'center' }}>
          <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="rgba(20,184,166,.4)" strokeWidth="1.5" strokeLinecap="round" style={{ display:'block', margin:'0 auto 14px' }}>
            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>
          </svg>
          <div style={{ fontSize: 14, fontWeight: 600, color: 'rgba(255,255,255,.6)', marginBottom: 8 }}>Awaiting Work Agreement</div>
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', lineHeight: 1.7 }}>
            The client has not yet created the work agreement. You will receive a notification once it is ready for your review and signature.
          </div>
        </div>
      )}

      {/* Case 2b  Agreement DRAFT  freelancer waiting for client to re-sign */}
      {ag && ag.status === 'DRAFT' && isFreelancer && (
        <div style={{ background: 'rgba(251,191,36,.06)', border: '1px solid rgba(251,191,36,.25)', borderRadius: 14, padding: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: '#fbbf24' }}>Agreement Updated  Awaiting Client Signature</div>
          </div>
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,.6)', lineHeight: 1.7, marginBottom: 14 }}>
            The client has revised the milestones or scope. They need to re-sign the updated agreement before sending it to you for review.
          </div>
          {ag.version > 1 && (
            <div style={{ padding: '8px 12px', borderRadius: 9, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.08)', fontSize: 12, color: 'rgba(255,255,255,.45)' }}>
              This is version {ag.version} of the agreement. You will be notified once the client submits the updated agreement for your review.
            </div>
          )}
        </div>
      )}
      ...
      {/* Case 9  AMENDMENT_PENDING  freelancer reviews milestone amendment */}
      {ag && ag.status === 'AMENDMENT_PENDING' && isFreelancer && (() => {
        ...
        const amendNote = (ag.specialConditions || '').replace('[AMENDMENT] ', '')

        const accept = async () => {
          setAccepting(true)
          try {
            await api.post('/agreements/' + jobId + '/amendment-accept')
        ...
        const reject = async () => {
          if (!rejectNote.trim()) { toast.error('Reason required'); return }
          setRejecting(true)
          try {
            await api.post('/agreements/' + jobId + '/amendment-reject', { rejectNote })
            toast.success('Amendment rejected  previous milestones restored.')
            fetch()
          } catch (e) { toast.error(errMsg(e)) }
          finally { setRejecting(false) }
        }
        return (
          <div style={{ background: 'rgba(129,140,248,.06)', border: '1px solid rgba(129,140,248,.25)', borderRadius: 14, padding: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#818cf8" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
              <div style={{ fontSize: 16, fontWeight: 700, color: '#818cf8' }}>Milestone Amendment  Action Required</div>
            </div>
            {amendNote && (
              <div style={{ padding: '10px 13px', borderRadius: 9, background: 'rgba(129,140,248,.08)', border: '1px solid rgba(129,140,248,.2)', fontSize: 13, color: 'rgba(255,255,255,.65)', marginBottom: 14 }}>
                <strong style={{ color: '#818cf8' }}>Reason for amendment:</strong> {amendNote}
              </div>
            )}
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,.35)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 10 }}>Updated Milestone Plan ({milestones.length})</div>
              {milestones.map((m, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,.06)', fontSize: 13 }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontWeight: 700, color: '#14b8a6' }}>Rs.{Number(m.amount || 0).toLocaleString('en-IN')}</div>
                    <div style={{ fontSize: 10, color: '#34d399', marginTop: 2 }}>Net Rs.{(Number(m.amount || 0) * 0.98).toFixed(0)}</div>
                  </div>
                </div>
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 8, fontWeight: 700, fontSize: 14 }}>
                ...
                <span style={{ color: '#14b8a6' }}>Rs.{total.toLocaleString('en-IN')}</span>
              </div>
            </div>
            {showReject && (
              <div style={{ marginBottom: 14 }}>
                <textarea
                  ...
                  style={{ width: '100%', padding: '9px 12px', background: 'rgba(255,255,255,.05)', border: '1.5px solid rgba(244,63,94,.3)', borderRadius: 9, color: '#f8fafc', fontSize: 13, fontFamily: 'inherit', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }}
                />
              </div>
            )}
            <div style={{ display: 'flex', gap: 10 }}>
              {showReject ? (
                <>
                  <button onClick={() => setShowReject(false)} style={{ padding: '10px 18px', borderRadius: 9, border: '1.5px solid rgba(255,255,255,.1)', background: 'transparent', color: 'rgba(255,255,255,.5)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14 }}>Back</button>
                  <button onClick={reject} disabled={rejecting} style={{ flex: 1, padding: '10px', borderRadius: 9, border: 'none', background: '#f43f5e', color: '#fff', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', fontSize: 14 }}>{rejecting ? 'Rejecting...' : 'Confirm Rejection'}</button>
                </>
              ) : (
                <>
                  <button onClick={() => setShowReject(true)} style={{ padding: '10px 18px', borderRadius: 9, border: '1px solid rgba(244,63,94,.3)', background: 'rgba(244,63,94,.08)', color: '#f43f5e', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 600 }}>Reject Amendment</button>
                  <button onClick={accept} disabled={accepting} style={{ flex: 1, padding: '10px', borderRadius: 9, border: 'none', background: '#34d399', color: '#07111d', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', fontSize: 14 }}>{accepting ? 'Accepting...' : 'Accept Amendment'}</button>
                </>
              )}
            </div>
          </div>
      ...
      {/* Case 10  AMENDMENT_PENDING  client waits */}
      {ag && ag.status === 'AMENDMENT_PENDING' && isClient && (
        <div style={{ background: 'rgba(129,140,248,.05)', border: '1px solid rgba(129,140,248,.2)', borderRadius: 14, padding: 24 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#818cf8', marginBottom: 8 }}>Amendment Sent  Awaiting Freelancer Review</div>
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,.55)', lineHeight: 1.65 }}>
            The updated milestone plan has been sent to the freelancer. They will accept or reject the amendment. Work continues at the current status once accepted.
          </div>
        </div>
      )}
