...
export default function JobDetailPage() {
  ...
  const fetchJob = async () => {
    try {
      ...
      setJob(data)
      // Also fetch agreement for Phase 2 modals (silent)
      try {
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          {/* Milestones */}
          {(isClient || isFreelancer) && (job.freelancerId || job.milestones?.length > 0) && (
            <div>
                ...
                <div style={{ marginBottom:10, background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.08)', borderRadius:12, padding:'12px 14px' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                    ...
                    <span style={{ fontSize:11, color:'#14b8a6' }}>
                      {(job.milestones?.filter(m => m.status === "RELEASED")?.length) || 0}/{job.milestones.length} done
                    </span>
                  </div>
                  {job.milestones.slice(0,3).map((m, i) => {
                    ...
                    return (
                      <div key={m.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'5px 0', borderBottom: i < Math.min(job.milestones.length,3)-1 ? '1px solid rgba(255,255,255,.05)' : 'none' }}>
                        ...
                        <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                          <span style={{ fontSize:11, fontWeight:600, color: m.isLocked ? '#475569' : statusColor }}>
                            {m.isLocked ? 'Locked' : m.status?.replace(/_/g,' ')}
                          </span>
                  ...
                  {job.milestones.length > 3 && (
                    <div style={{ fontSize:11, color:'rgba(255,255,255,.35)', marginTop:5, textAlign:'center' }}>
                      +{job.milestones.length - 3} more milestones
                    </div>
                  )}
                </div>
              )}
              ...
              >
                {job.milestones?.length > 0 ? 'Manage Milestones' : 'View Milestones'}
              </Link>
            </div>
          )}
