...
export function useRegistration(role) {
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      // Update stored user so ProtectedRoute sees profileCompleted=true
      ...
      if (status === 400 && Object.keys(backendErrs).length > 0) {
        // Show each field error in the form
        setErrors(prev => ({ ...prev, ...backendErrs }))
        // Also show a toast with the first error so user knows what happened
        const firstError = Object.values(backendErrs)[0]
        toast.error(firstError || 'Please fix the highlighted fields')
      } else if (status === 409) {
        toast.error(msg)   // duplicate email/phone
      } else {
        toast.error(msg)
      }
      return { ok: false, errors: backendErrs }
    } finally {
      setSaving(false)
    }
  }, [data])
