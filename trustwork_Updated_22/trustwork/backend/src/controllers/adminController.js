...
const getEscrows = async (req, res) => {
  try {
    ...
    const where = { milestoneId: null }  // job-level escrows
    if (status) where.status = status
          ...
          job: {
            select: {
              ...
              milestones: {
                ...
                select: { id: true, title: true, amount: true, status: true, payoutStatus: true },
              },
            },
          },
        },
...
//  GET /admin/milestone-escrows - milestone-level escrows for admin review
const getMilestoneEscrows = async (req, res) => {
  try {
    const { status, jobId } = req.query
    const where = { milestoneId: { not: null } }
    if (status) where.status = status
      ...
      include: {
        milestone: {
          select: {
            ...
            description: true, deliverable: true, dueDate: true,
            submittedAt: true, submissionNote: true,
            clientApprovedAt: true, releasedAt: true, rejectedAt: true,
            rejectReason: true, reworkNote: true, reworkCount: true,
            platformFee: true, netAmount: true, payoutStatus: true,
            payoutQueue: {
              select: { id: true, status: true, grossAmount: true, platformFee: true, netAmount: true, createdAt: true, reviewedAt: true, adminNote: true },
            },
          },
        },
        job: {
    ...
    // Group by job for the Escrow tab view
    const byJob = {}
    for (const e of escrows) {
      const jid = e.jobId
      if (!byJob[jid]) byJob[jid] = { job: e.job, escrows: [] }
      byJob[jid].escrows.push(e)
    }

    res.json({ escrows, byJob: Object.values(byJob), total: escrows.length })
  } catch (err) { res.status(500).json({ error: err.message }) }
}

//  GET /admin/payout-pending - milestones in PAYMENT_UNDER_REVIEW (quick stat for badge)
const getPayoutPending = async (req, res) => {
  try {
    const count = await prisma.payoutQueue.count({ where: { status: 'PENDING_REVIEW' } })
    const total = await prisma.payoutQueue.aggregate({
      where: { status: 'PENDING_REVIEW' },
      _sum: { netAmount: true, grossAmount: true, platformFee: true },
    })
    res.json({
      count,
      totalGross: Number(total._sum.grossAmount || 0),
      totalFee:   Number(total._sum.platformFee  || 0),
      totalNet:   Number(total._sum.netAmount     || 0),
    })
  } catch (err) { res.status(500).json({ error: err.message }) }
}
