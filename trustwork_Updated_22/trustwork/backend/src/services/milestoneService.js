...
async function unlockFirstMilestone(jobId) {
  ...
  // Notify freelancer
  const job = await prisma.job.findUnique({ where: { id: jobId }, select: { id: true, title: true, freelancerId: true } })
  if (job?.freelancerId) {
    await N.milestoneUnlocked(job.freelancerId, { id: jobId, title: job.title }, first.title)
  }
  return updated
}
...
async function getMilestones(jobId, userId) {
  ...
  const enriched = milestones.map(m => ({
    ...
    canSubmit:     !isFreelancer ? false : ['FUNDED','IN_PROGRESS','REWORK_REQUESTED'].includes(m.status) && !m.isLocked,
  ...
  const summary = milestones.reduce((acc, m) => {
    ...
    acc.inProgress     += m.status === 'IN_PROGRESS' ? a : 0
    acc.reworkRequested += m.status === 'REWORK_REQUESTED' ? a : 0
    acc.clientApproved += m.status === 'CLIENT_APPROVED' ? a : 0
    acc.paymentReview  += m.status === 'PAYMENT_UNDER_REVIEW' ? a : 0
    acc.underReview    += ['UNDER_ADMIN_REVIEW','PAYMENT_UNDER_REVIEW'].includes(m.status) ? a : 0
    acc.released       += m.status === 'RELEASED' ? a : 0
    acc.refunded       += m.status === 'REFUNDED' ? a : 0
    acc.count     += 1
    acc.released_count += m.status === 'RELEASED' ? 1 : 0
    return acc
  }, { total:0, locked:0, unlocked:0, funded:0, inProgress:0, submitted:0, reworkRequested:0, clientApproved:0, paymentReview:0, underReview:0, released:0, refunded:0, rejected:0, count:0, released_count:0 })
...
async function editMilestone(milestoneId, userId, fields) {
  ...
  if (agreement && ['ACTIVE','CLIENT_SIGNED'].includes(agreement.status)) {
    ...
    if (m.job.freelancerId) {
      await N.milestoneEdited(m.job.freelancerId, { id: m.jobId, title: m.job.title }, m.title)
    }
  }
...
async function submitMilestone(milestoneId, userId, submissionNote) {
  ...
  if (!['FUNDED', 'IN_PROGRESS', 'REWORK_REQUESTED'].includes(m.status))
    throw { status: 400, message: 'Cannot submit at current status: ' + m.status }
...
async function clientApproveMilestone(milestoneId, userId) {
  ...
  // Notify freelancer: payment under review
  await N.milestoneApproved(m.job.freelancerId, { id: m.job.id, title: m.job.title }, m.title, net)
  await N.paymentUnderReview(m.job.freelancerId, { id: m.job.id, title: m.job.title }, m.title)
...
async function clientRejectMilestone(milestoneId, userId, rejectReason) {
  ...
  // Notify freelancer
  await N.milestoneRejected(m.job.freelancerId, { id: m.job.id, title: m.job.title }, m.title, rejectReason)
...
async function adminReleasePayout(payoutId, adminId, adminNote) {
  ...
  // Notify freelancer of payout release
  await N.payoutReleased(m.job.freelancerId, { id: m.job.id, title: m.job.title }, m.title, net)
...
async function refundMilestone(milestoneId, userId) {
  ...
  if (!['FUNDED','IN_PROGRESS','SUBMITTED','REWORK_REQUESTED','PAYMENT_UNDER_REVIEW','UNDER_ADMIN_REVIEW','CLIENT_APPROVED'].includes(m.status))
...
//  START work (freelancer) FUNDED -> IN_PROGRESS 
async function startWork(milestoneId, userId) {
  const m = await getMilestoneWithJob(milestoneId)
  if (m.job.freelancerId !== userId)
    throw { status: 403, message: 'Only the assigned freelancer can start milestones' }
  if (m.isLocked)
    throw { status: 400, message: 'Milestone is locked' }
  if (m.status !== 'FUNDED')
    throw { status: 400, message: 'Only FUNDED milestones can be started. Current: ' + m.status }
  ...
  await N.milestoneStarted(m.job.clientId, { id: m.job.id, title: m.job.title }, m.title)

  return { message: `"${m.title}" started  work in progress`, milestone: updated }
}


//  CLIENT requests rework SUBMITTED -> REWORK_REQUESTED 
async function requestRework(milestoneId, userId, reworkNote) {
  const m = await getMilestoneWithJob(milestoneId)
  if (m.job.clientId !== userId)
    throw { status: 403, message: 'Only the client can request rework' }
  if (m.status !== 'SUBMITTED')
    throw { status: 400, message: 'Only SUBMITTED milestones can have rework requested. Current: ' + m.status }
  if (!reworkNote?.trim())
    throw { status: 400, message: 'Rework instructions are required' }
  ...
  await N.reworkRequested(m.job.freelancerId, { id: m.job.id, title: m.job.title }, m.title, reworkNote)

  return { message: `Rework requested for "${m.title}". Freelancer notified.`, milestone: updated }
}
