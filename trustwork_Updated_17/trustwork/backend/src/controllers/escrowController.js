...
const PLATFORM_FEE_RATE = 0.02  // 2% platform fee deducted from freelancer payout
...
const fundEscrow = async (req, res) => {
  try {
    ...
    // Phase 1: Work Agreement gate -- both parties must have signed before funding
    const agreement = await prisma.workAgreement.findUnique({ where: { jobId } });
    if (!agreement || agreement.status !== 'ACTIVE') {
      return res.status(400).json({
        error: 'Both parties must complete and sign the Work Agreement before funding escrow.',
    ...
    const [escrow, updatedJob] = await prisma.$transaction([
      prisma.escrow.create({
        data: {
          jobId, amount: job.budget, status: 'LOCKED',
          paymentId, orderId, milestoneId: null,
          platformFeeRate: PLATFORM_FEE_RATE,  // recorded at time of funding
        },
      }),
    ...
    // Notify freelancer work can begin
    if (job.freelancerId) await N.funded(job.freelancerId, updatedJob)
    ...
    const [updatedEscrow] = await prisma.$transaction([
      ...
      prisma.transaction.create({
        data: {
          ...
          description: `Escrow released. TrustWork fee: Rs.${fee} (${(PLATFORM_FEE_RATE*100)}%). You receive: Rs.${net}`,
        },
      }),
    ]);

    res.json({
      message: `Payment released. TrustWork fee: Rs.${fee}. Freelancer receives: Rs.${net}`,
      escrow: updatedEscrow, gross, platformFee: fee, netPayout: net,
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
