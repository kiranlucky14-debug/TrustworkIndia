...
export function Modal({ open, onClose, title, children, wide = false }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative card w-full animate-slide-up ${wide ? 'max-w-2xl' : 'max-w-lg'}`}
        style={{ maxHeight: '90vh', overflowY: 'auto' }}>
        <div className="flex items-center justify-between p-5 border-b border-ink-800" style={{ position: 'sticky', top: 0, background: 'inherit', zIndex: 1 }}>
          <h3 className="font-display font-semibold text-lg text-white">{title}</h3>
          <button onClick={onClose} className="btn-ghost btn-sm rounded-lg">
