...
const TABS = [
  ...
  { id:'health',       label:'System Health', icon:'M22 12h-4l-3 9L9 3l-3 9H2' },
  { id:'settings',     label:'Settings',      icon:'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm-7-3a7 7 0 1 0 14 0A7 7 0 0 0 5 12z' },
]
...
export default function AdminDashboard() {
  ...
  const [cfgDirty,   setCfgDirty]   = useState({})
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (tab === 'settings') {
        const { data: cfgRes } = await api.get('/admin/config')
        setSysConfig(cfgRes.config || {})
        setLoading(false)
        return
      }
      if (tab === 'health') {
        const { data: hRes } = await api.get('/admin/config/health')
        setSysHealth(hRes)
        setLoading(false)
        return
      }
        ...
        <main style={{ flex:1, padding:24, overflow:'auto' }}>
          ...
          {tab === 'health' && (
            <div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24, flexWrap:'wrap', gap:12 }}>
                <div>
                  <h2 style={{ fontSize:20, fontWeight:700, color:'#f8fafc', margin:0 }}>System Health</h2>
                  <p style={{ fontSize:13, color:'rgba(255,255,255,.45)', marginTop:4 }}>Live status of all platform services and integrations</p>
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  {sysHealth && <div style={{ padding:'4px 14px', borderRadius:100, background:sysHealth.status==='healthy'?'rgba(52,211,153,.15)':'rgba(251,146,60,.15)', color:sysHealth.status==='healthy'?'#34d399':'#fb923c', fontWeight:700, fontSize:13, border:`1px solid ${sysHealth.status==='healthy'?'rgba(52,211,153,.3)':'rgba(251,146,60,.3)'}` }}>{sysHealth.status?.toUpperCase()}</div>}
                  <button onClick={fetchData} className="btn-secondary btn text-sm px-3 py-1.5">Refresh</button>
                </div>
              </div>
              {sysHealth && (
                <>
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:10, marginBottom:16 }}>
                    {[
                      { label:'Database',     val:`${sysHealth.checks?.database?.status} (${sysHealth.checks?.database?.latency_ms}ms)`, ok:sysHealth.checks?.database?.status==='ok', icon:'' },
                      { label:'OTP Provider', val:sysHealth.checks?.otp_provider?.provider,  warn:sysHealth.checks?.otp_provider?.status==='mock', ok:sysHealth.checks?.otp_provider?.status!=='mock', icon:'' },
                      { label:'Email',        val:sysHealth.checks?.email_provider?.provider, warn:sysHealth.checks?.email_provider?.status==='mock', ok:sysHealth.checks?.email_provider?.status!=='mock', icon:'' },
                      { label:'Payments',     val:sysHealth.checks?.payment?.mode, warn:sysHealth.checks?.payment?.mode!=='live', ok:sysHealth.checks?.payment?.mode==='live', icon:'' },
                      { label:'Platform Fee', val:sysHealth.checks?.platform_fee?.percent, ok:true, icon:'' },
                      { label:'Environment',  val:sysHealth.checks?.environment, warn:sysHealth.checks?.environment!=='production', ok:sysHealth.checks?.environment==='production', icon:'' },
                      { label:'Uptime',       val:`${Math.floor((sysHealth.checks?.uptime_seconds||0)/3600)}h ${Math.floor(((sysHealth.checks?.uptime_seconds||0)%3600)/60)}m`, ok:true, icon:'' },
                      { label:'Memory',       val:`${sysHealth.checks?.memory_mb} MB`, warn:(sysHealth.checks?.memory_mb||0)>=512, ok:(sysHealth.checks?.memory_mb||0)<512, icon:'' },
                    ].map(({ label, val, ok, warn, icon }) => (
                      <div key={label} style={{ padding:'14px 16px', borderRadius:12, background:ok?'rgba(52,211,153,.05)':warn?'rgba(251,146,60,.05)':'rgba(255,255,255,.04)', border:`1px solid ${ok?'rgba(52,211,153,.2)':warn?'rgba(251,146,60,.2)':'rgba(255,255,255,.08)'}` }}>
                        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:5 }}>
                          <span style={{ fontSize:16 }}>{icon}</span>
                          <span style={{ fontSize:11, color:'rgba(255,255,255,.38)', textTransform:'uppercase', letterSpacing:'.07em' }}>{label}</span>
                          <span style={{ marginLeft:'auto' }}>{ok?'':warn?'':''}</span>
                        </div>
                        <div style={{ fontSize:14, fontWeight:700, color:ok?'#34d399':warn?'#fb923c':'#f43f5e' }}>{val}</div>
                      </div>
                    ))}
                  </div>
                  <div className="card p-5">
                    <h3 style={{ fontSize:14, fontWeight:600, color:'#f8fafc', marginBottom:12 }}>Feature Flags</h3>
                    <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                      {Object.entries(sysHealth.checks?.features||{}).map(([k,v]) => (
                        <div key={k} style={{ padding:'5px 14px', borderRadius:100, background:v?'rgba(52,211,153,.12)':'rgba(255,255,255,.06)', border:`1px solid ${v?'rgba(52,211,153,.3)':'rgba(255,255,255,.1)'}`, color:v?'#34d399':'rgba(255,255,255,.4)', fontSize:13, fontWeight:600 }}>
                          {k.replace('_',' ')} {v?'ON':'OFF'}
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          {tab === 'settings' && (
            <div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24, flexWrap:'wrap', gap:12 }}>
                <div>
                  <h2 style={{ fontSize:20, fontWeight:700, color:'#f8fafc', margin:0 }}>System Settings</h2>
                  <p style={{ fontSize:13, color:'rgba(255,255,255,.45)', marginTop:4 }}>Configure platform behaviour  changes apply instantly without server restart</p>
                </div>
                <button onClick={async () => {
                  if (!Object.keys(cfgDirty).length) { toast.error('No changes to save'); return }
                  setCfgSaving(true)
                  try {
                    await api.patch('/admin/config', cfgDirty)
                    toast.success('Settings saved!')
                    setCfgDirty({})
                    fetchData()
                  } catch(e) { toast.error(errMsg(e)) }
                  finally { setCfgSaving(false) }
                }} disabled={cfgSaving || !Object.keys(cfgDirty).length}
                  style={{ padding:'9px 22px', borderRadius:10, border:'none', background:Object.keys(cfgDirty).length?'#14b8a6':'rgba(255,255,255,.12)', color:Object.keys(cfgDirty).length?'#07111d':'rgba(255,255,255,.3)', fontWeight:700, fontSize:14, cursor:Object.keys(cfgDirty).length?'pointer':'not-allowed', fontFamily:'inherit' }}>
                  {cfgSaving ? 'Saving...' : `Save Changes${Object.keys(cfgDirty).length?' ('+Object.keys(cfgDirty).length+')':''}`}
                </button>
              </div>

              {[
                { group:' Critical  Production Checklist', warn:true, fields:[
                  { key:'MAINTENANCE_MODE', label:'Maintenance Mode', type:'toggle', desc:'Take site offline for all users (admin still accessible)' },
                  { key:'MAINTENANCE_MESSAGE', label:'Maintenance Message', type:'text', desc:'Message shown to users during maintenance' },
                  { key:'OTP_PROVIDER', label:'OTP Provider', type:'select', options:['mock','msg91','twilio','fast2sms'], desc:' mock uses 123456  NOT safe for production' },
                  { key:'OTP_MOCK_CODE', label:'Mock OTP Code', type:'text', desc:'Code used in mock/dev mode only' },
                  { key:'EMAIL_PROVIDER', label:'Email Provider', type:'select', options:['mock','smtp','sendgrid','resend'], desc:' mock skips all emails' },
                  { key:'RAZORPAY_MODE', label:'Payment Mode', type:'select', options:['mock','test','live'], desc:' Set to live for real money transactions' },
                  { key:'FEATURE_RAZORPAY', label:'Enable Real Razorpay', type:'toggle', desc:'Show real payment checkout' },
                ]},
                { group:' Platform & Fees', fields:[
                  { key:'PLATFORM_FEE_RATE', label:'Platform Fee Rate', type:'text', desc:'e.g. 0.02 = 2% per milestone payout' },
                  { key:'MIN_MILESTONE_AMOUNT', label:'Min Milestone (Rs.)', type:'text', desc:'Minimum milestone amount allowed' },
                  { key:'MAX_MILESTONE_AMOUNT', label:'Max Milestone (Rs.)', type:'text', desc:'Maximum milestone amount allowed' },
                ]},
                { group:' Security', fields:[
                  { key:'RATE_LIMIT_MAX', label:'Rate Limit (req/15min/IP)', type:'text', desc:'Max API requests per window per IP' },
                  { key:'OTP_RATE_LIMIT', label:'OTP Rate Limit (per hour)', type:'text', desc:'Max OTP requests per phone per hour' },
                  { key:'JWT_EXPIRY', label:'JWT Token Expiry', type:'text', desc:'e.g. 7d, 24h, 30m' },
                ]},
                { group:' Feature Flags', fields:[
                  { key:'FEATURE_CHAT', label:'In-Platform Chat', type:'toggle', desc:'Enable real-time job messaging' },
                  { key:'FEATURE_MATCHING', label:'Job Matching', type:'toggle', desc:'Show matched jobs on dashboard' },
                  { key:'FEATURE_DISPUTES', label:'Dispute System', type:'toggle', desc:'Allow users to raise disputes' },
                  { key:'FEATURE_PUBLIC_PROFILE', label:'Public Profiles', type:'toggle', desc:'Enable shareable freelancer profile pages' },
                ]},
                { group:' Email Notifications', fields:[
                  { key:'NOTIFY_PAYMENT_EMAIL', label:'Payment Released Email', type:'toggle', desc:'Email freelancer on payout release' },
                  { key:'NOTIFY_DISPUTE_EMAIL', label:'Dispute Email', type:'toggle', desc:'Email parties when dispute is raised' },
                  { key:'NOTIFY_SIGNUP_EMAIL', label:'Welcome Email', type:'toggle', desc:'Send welcome email on registration' },
                ]},
                { group:' Support', fields:[
                  { key:'SUPPORT_EMAIL', label:'Support Email', type:'text', desc:'Shown to users for help' },
                  { key:'SUPPORT_PHONE', label:'Support Phone', type:'text', desc:'WhatsApp/phone support number' },
                ]},
              ].map(({ group, warn, fields }) => (
                <div key={group} className="card p-5 mb-4">
                  <h3 style={{ fontSize:14, fontWeight:700, color:warn?'#fb923c':'#f8fafc', marginBottom:16, display:'flex', alignItems:'center', gap:8 }}>
                    {group}
                    {warn && <span style={{ fontSize:10, padding:'2px 8px', borderRadius:100, background:'rgba(251,146,60,.12)', color:'#fb923c', border:'1px solid rgba(251,146,60,.25)' }}>Action Required for Production</span>}
                  </h3>
                  <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                    {fields.map(({ key, label, type, desc, options }) => {
                      const current = cfgDirty[key] !== undefined ? cfgDirty[key] : (sysConfig[key] ?? '')
                      const isDirty = cfgDirty[key] !== undefined
                      const onChange = v => setCfgDirty(prev => ({ ...prev, [key]: v }))
                      return (
                        <div key={key} style={{ display:'flex', alignItems:'flex-start', gap:16, paddingBottom:14, borderBottom:'1px solid rgba(255,255,255,.06)' }}>
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                              <label style={{ fontSize:13, fontWeight:600, color:'#e2e8f0' }}>{label}</label>
                              {isDirty && <span style={{ fontSize:10, padding:'1px 6px', borderRadius:100, background:'rgba(251,191,36,.15)', color:'#fbbf24', border:'1px solid rgba(251,191,36,.3)' }}>Modified</span>}
                            </div>
                            <div style={{ fontSize:11, color:'rgba(255,255,255,.35)', lineHeight:1.5 }}>{desc}</div>
                          </div>
                          <div style={{ flexShrink:0, minWidth:180 }}>
                            {type === 'toggle' ? (
                              <button type="button" onClick={() => onChange(current==='true'?'false':'true')}
                                style={{ position:'relative', width:44, height:24, borderRadius:12, border:'none', background:current==='true'?'#14b8a6':'rgba(255,255,255,.15)', cursor:'pointer', transition:'all .2s', outline:'none' }}>
                                <span style={{ position:'absolute', top:3, left:current==='true'?22:3, width:18, height:18, borderRadius:'50%', background:'#fff', transition:'left .2s', boxShadow:'0 1px 3px rgba(0,0,0,.3)' }}/>
                              </button>
                            ) : type === 'select' ? (
                              <select value={current} onChange={e => onChange(e.target.value)}
                                style={{ padding:'7px 10px', borderRadius:8, background:'rgba(255,255,255,.06)', border:`1.5px solid ${isDirty?'#fbbf24':'rgba(255,255,255,.12)'}`, color:'#f8fafc', fontSize:13, fontFamily:'inherit', outline:'none', cursor:'pointer', width:'100%' }}>
                                {(options||[]).map(o => <option key={o} value={o} style={{ background:'#0e1c2f' }}>{o}</option>)}
                              </select>
                            ) : (
                              <input value={current} onChange={e => onChange(e.target.value)}
                                style={{ padding:'7px 10px', borderRadius:8, background:'rgba(255,255,255,.06)', border:`1.5px solid ${isDirty?'#fbbf24':'rgba(255,255,255,.12)'}`, color:'#f8fafc', fontSize:13, fontFamily:'inherit', outline:'none', width:'100%', boxSizing:'border-box' }}/>
                            )}
                          </div>
                        </div>
                      )
              ...
              <div className="card p-5">
                <h3 style={{ fontSize:14, fontWeight:600, color:'#f8fafc', marginBottom:14 }}> Test Integrations</h3>
                <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                  {[
                    { label:'Test OTP send', action: async () => { const r = await api.post('/admin/config/test-otp',{phone:'9876543210'}); toast.success(r.data.message) }},
                    { label:'Test email send', action: async () => { const r = await api.post('/admin/config/test-email',{email:sysConfig.SUPPORT_EMAIL||'test@example.com'}); toast.success(r.data.message) }},
                    { label:'View system health', action: () => { setActiveTab('health'); fetchData() }},
                  ].map(({ label, action }) => (
                    <button key={label} type="button" onClick={async () => { try { await action() } catch(e) { toast.error(errMsg(e)) } }}
                      style={{ padding:'8px 18px', borderRadius:9, border:'1px solid rgba(255,255,255,.15)', background:'rgba(255,255,255,.06)', color:'rgba(255,255,255,.7)', fontSize:13, cursor:'pointer', fontFamily:'inherit', fontWeight:600, transition:'all .18s' }}>
                      {label}
                    </button>
                  ))}
                </div>
          ...
          {/* Pagination */}
          {!['overview','payouts','settings','health'].includes(tab) && totalPages > 1 && (
