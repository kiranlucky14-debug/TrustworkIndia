...
export default function JobDetailPage() {
  ...
  const fetchJob = async () => {
    try {
      ...
      setJob(data)
      // Fetch agreement for Phase 2 modals (silent)
      try {
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {/* Milestone Panel  shown inline for assigned/active jobs */}
          {(isClient || isFreelancer || user?.role === 'ADMIN') && !['CREATED'].includes(job.status) && (
            <div className="card p-5">
              <MilestonePanel jobId={job.id} job={job} user={user} />
            </div>
          )}
          ...
          {isClient && (
            <div className="card p-5 space-y-3">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {/* Submit/Approve/Reject moved to per-milestone actions in MilestonePanel below */}
              </div>
            </div>
          )}

          {/* Per-milestone submit is handled inside MilestonePanel */}
        </div>
        ...
        <div className="space-y-4">
          ...
          {/* Chat  open for assigned job parties */}
          {(isClient || isFreelancer) && job.freelancerId && (
            <Link
              to={`/chat/${job.id}`}
              className="btn w-full text-center"
              style={{ background:'rgba(20,184,166,.12)', border:'1px solid rgba(20,184,166,.3)', color:'#14b8a6', padding:'10px', borderRadius:10, textDecoration:'none', display:'flex', alignItems:'center', justifyContent:'center', gap:8, fontWeight:600, fontSize:14 }}>
          ...
          {/* Milestones - full panel now in main content, just a link here */}
          {(isClient || isFreelancer || user?.role === 'ADMIN') && job.status !== 'CREATED' && (
            <Link
      ...
      {/* Modals */}
      {/* Phase 2: Submit confirmation (legacy - kept for agreement-based flow) */}
      <SubmitConfirmModal
